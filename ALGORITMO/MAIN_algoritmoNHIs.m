...
%QUESTO ALGORITMO HA LA FINALITà DI CALCOLARE:
...
I = imread(filename); I = im2double(I);

%Caricamento dell'immagine target:
Itarget = imread(path+"TARGET.jpg"); Itarget = im2double(Itarget);
...
[W_mt,W_bt,BW]=segmentation(Itarget);
...
%Calcolo di rSE:
rse=rSE(IsNorm,file,path);

%Rappresentazione grafica dell'immagine target, immagine source originale e
%normalizzata:
figure(1);
ax1=subplot(131);imshow(Itarget); title('Immagine target');
ax2=subplot(132);imshow(I); title('Immagine source originale');
ax3=subplot(133);imshow(IsNorm); title('Immagine source normalizzata'); 
sgtitle(['Immagine: ' file ' con rSE: ' num2str(rse) '%']);
linkaxes([ax1 ax2 ax3]);

%Rappresentazione grafica dei punti scelti sull'immagine source per il
%calcolo dell rSE: 
figure(2);
sour=load(path+"punti_"+file(1:end-4)+".mat");
Is=imread(filename);
Is=im2double(Is);
ax1=subplot(121);imshow(Is);hold on;plot(sour.tot_brown(:,1),sour.tot_brown(:,2),'*r');plot(sour.tot_blue(:,1),sour.tot_blue(:,2),'*b'); title('Immagine source originale');
ax2=subplot(122);imshow(IsNorm);hold on;plot(sour.tot_brown(:,1),sour.tot_brown(:,2),'*r');plot(sour.tot_blue(:,1),sour.tot_blue(:,2),'*b'); title('Immagine source normalizzata');
sgtitle("Punti scelti manualmente su immagine source");
linkaxes([ax1 ax2]);
